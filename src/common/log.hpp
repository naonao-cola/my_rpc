/**
 * @FilePath     : /my_rpc/src/common/log.hpp
 * @Description  :
 * @Author       : naonao
 * @Date         : 2025-05-01 14:29:51
 * @Version      : 0.0.1
 * @LastEditors  : naonao
 * @LastEditTime : 2025-05-01 14:29:52
...
public:
    ...
    void Log(std::string logId, LogLevel level, char* format, ...)
    {
        ...
        std::string logMsg  = levelStr(level) + " " + timeStr + " " + std::to_string(getpid()) + "," + logId + " " + buf + "\n";
        free(buf);
        RobustIo io(fd_);
...
#define TRACE(format, ...) LOGGER.Log("", Common::LEVEL_TRACE, (char*)"(%s:%s:%d):" format, FILENAME(__FILE__), __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define DEBUG(format, ...) LOGGER.Log("", Common::LEVEL_DEBUG, (char*)"(%s:%s:%d):" format, FILENAME(__FILE__), __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define INFO(format, ...) LOGGER.Log("", Common::LEVEL_INFO, (char*)"(%s:%s:%d):" format, FILENAME(__FILE__), __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define WARN(format, ...) LOGGER.Log("", Common::LEVEL_WARN, (char*)"(%s:%s:%d):" format, FILENAME(__FILE__), __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define ERROR(format, ...) LOGGER.Log("", Common::LEVEL_ERROR, (char*)"(%s:%s:%d):" format, FILENAME(__FILE__), __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define CTX_TRACE(ctx, format, ...)                      \
    LOGGER.Log(ctx.log_id(),                             \
               Common::LEVEL_TRACE,                      \
               (char*)"(%d:%s:%s:%d):" format,           \
               MyCoroutine::ScheduleGetRunCid(SCHEDULE), \
               FILENAME(__FILE__),                       \
               __FUNCTION__,                             \
               __LINE__,                                 \
               ##__VA_ARGS__)
